...
VERTICAL_LABEL_PAD      = 0.9     # extra push for extrinsic labels on vertical links
...
CIRCLE_HEAD_RADIUS      = 0.18    # radius of little circle arrow-heads
CIRCLE_POINTER_SIZE     = CIRCLE_HEAD_RADIUS * 2  # diameter for pointer size
...
def draw_self_loop(ax, position, loop_radius=0.4, color='#008080',
    ...
    """
    Draw a circular self-loop with a filled arrow-head.
    """
    cx, cy = position[0] + offset[0], position[1] + offset[1]
    loop = Circle((cx, cy), loop_radius,
...
def process_file(mat_file):
    """
    Load a .mat file and extract extrinsic, intrinsic, and shape segments.
    """
...
def plot_covariate(ax, cov, data, node_images, shape_images):
    """
    Plot extrinsic, intrinsic, and shape connectivity for one covariate.
    """
    ...
    for pos in REGION_POSITIONS.values():
        ax.add_patch(Circle(pos, REGION_RADIUS, facecolor='none', edgecolor='black', lw=0, zorder=1))
    ...
    for area, items in shapes_by_area.items():
        ...
        for (shape_idx, ep_val), angle in zip(items_sorted, angles):
            ...
            if ep_val != 0:
                ...
                ax.text(x0, y0+dy_arrow, fmt_trunc_str(ep_val,2), color=col,
    ...
    for roi in REGION_POSITIONS:
        ...
        pos = {
            n: tuple(np.array(REGION_POSITIONS[roi]) +
        ...
        for u,_,d in selfe:
            p = pos[u]
            off = SELF_LOOP_OFFSETS.get(u, (0,0))
            col = '#FF0056' if d['Ep']>0 else '#008080'
...
def main():
    ...
    parser.add_argument('--mat-files', '-m', nargs='+', required=True,
                        help='Paths to .mat files')
    parser.add_argument('--outdir', '-o', required=True,
                        help='Directory to save figures')
    parser.add_argument('--node-images', nargs=4,
                        metavar=('SS','SP','II','DP'),
                        help='Override paths to node icon images')
    parser.add_argument('--shape-images', nargs='+',
    ...
    node_imgs = (dict(enumerate(args.node_images, start=1))
                 if args.node_images else NODE_IMAGES_DEFAULT)
    shape_imgs = (dict(enumerate(args.shape_images, start=1))
